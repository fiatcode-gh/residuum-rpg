...
green, and is one conventional commit authored
`fiatcode <fiatcode@posteo.com>`.
...
* `Item.temper` (int, default 0); `attackMin`/`attackMax` add it for weapons,
  `armor` adds it for armour; `maxHp`/`speed` untouched; `props` gains it;
...
* `craft/craft.dart`: `smeltCost = 2`, `brewCost = 3`, `smeltRefusal`,
  `brewRefusal`.
...
terrain and items, its own painter constant, not a palette field),
`event_messages` (`NodeGathered` arm), `inventory_screen` (Materials panel, fixed
