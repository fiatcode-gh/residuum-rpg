...
```bash
git add packages/core && git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: give Rng an exportable state on a splitmix64 core"
```
...
```bash
cd /var/home/dhemas/Development/Projects/fiatcode/residuum-rpg/.worktrees/m3-rng && git add -A && git commit --author="fiatcode <fiatcode@posteo.com>" -m "test: re-pin seeded outcomes onto the new generator stream"
```
...
```bash
cd /var/home/dhemas/Development/Projects/fiatcode/residuum-rpg/.worktrees/m3-rng && git add BUILD-REPORT.md docs/superpowers/plans && git commit --author="fiatcode <fiatcode@posteo.com>" -m "docs: record the M3R build report"
```
