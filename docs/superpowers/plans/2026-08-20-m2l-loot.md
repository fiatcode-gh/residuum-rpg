...
- Every commit's exit state is green. Conventional commits,
  `--author="fiatcode <fiatcode@posteo.com>"`. Never push, never open a PR.
...
no assertion changes: `packages/core/test/support/fixtures.dart` (`crawl`),
`packages/app/test/game_bloc_test.dart` (`arenaGame`),
`packages/content/test/content_validation_test.dart` (the 1→5 descent test), and
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(loot): equip slots and rarity tiers"
```
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(loot): base items, affixes and rolled items"
```
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(skills): learn-by-doing levels and the rising cost curve"
```
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(loot): derive effective hero stats from gear and training"
```
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(loot): weighted drop tables and the roll that reads them"
```
...
- Produces: `GameState` gains required `lootRng` and optional `groundItems`,
  `inventory`, `equipment`, `skills`, `dropTables`, `nextDropNumber`, plus
  ...
  it. Actions: `PickUpAction`, `EquipAction(String itemId)`,
  `UnequipAction(EquipSlot slot)`, `DrinkAction(String itemId)`,
  `DropAction(String itemId)`. Events: `ItemDropped(Item item, Position at)`,
  `ItemPickedUp(Item item)`, `InventoryFull()`, `ItemEquipped(Item, EquipSlot)`,
  `ItemUnequipped(Item, EquipSlot)`, `EquipRefused(String reason)`,
  `PotionDrunk(Item, int healed)`, `AttackDodged(String attackerId)`,
  `SkillLevelledUp(SkillId skill, int level)`.
...
`groundItems = const {}`, `inventory = const []`, `equipment = const {}`,
`Map<SkillId, SkillState>? skills`, `dropTables = const {}`,
`nextDropNumber = 1`; wrap the collections with `Map.unmodifiable` /
...
`fixtures.dart`: `crawl` gains `lootSeed`, `groundItems`, `inventory`,
`equipment`, `skills`, `dropTables` parameters, all defaulted, and passes
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(engine): carry loot, gear and skills in the game state"
```
...
```bash
git add packages/core
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(combat): armour, dodge and learn-by-doing in the damage math"
```
...
- Produces: `step` handles `PickUpAction`, `EquipAction`, `UnequipAction`,
  `DrinkAction`, `DropAction`. A refused action returns the state unchanged
...
```bash
git add packages/core
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(loot): pick up, equip, unequip, drink and drop"
```
...
- Produces: `generateFloor` gains `required int itemCount` and
  ...
  `state.lootRng` against its `dropChance` and, on success,
  `rollDrop(table, state.lootRng, 'drop-<n>')` onto the death tile with
  `ItemDropped`.
...
```bash
git add packages/core
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(loot): floors scatter items and kills drop them"
```
...
- Produces: `const List<BaseItem> armory` plus a named const per item;
  `BaseItem baseItemById(String id)`; `const List<Affix> affixPool`,
  `weaponAffixes`, `armourAffixes`; `Map<int, DropTable> dropTables` and
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(content): armory, affix pool, drop tables and the starting kit"
```
...
```bash
git add packages/content
git commit --author="fiatcode <fiatcode@posteo.com>" -m "test(content): validate the armory, affix pools and drop tables"
```
...
```bash
git add packages/content
git commit --author="fiatcode <fiatcode@posteo.com>" -m "test(content): a bot proves the descent survivable, and the tuning that got it there"
```
...
- `game_bloc.dart`: `PickUpPressed`, `EquipPressed(itemId)`,
  `UnequipPressed(slot)`, `DrinkPressed(itemId)`, `DropPressed(itemId)`,
  `QuickDrinkPressed`. `GameViewState` gains `bool get canPickUp` and
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat(app): inventory, equipment and skills on screen"
```
...
```bash
git add packages/app
git commit --author="fiatcode <fiatcode@posteo.com>" -m "test(app): loot, equipment and training reach the view"
```
...
**Type consistency.** `Loadout`, `Equipment`, `heroAttack`, `heroArmor`,
`heroDodgePercent`, `heroMaxHp`, `heroSpeed`, `Weighted`, `DropTable`,
`rollDrop`, `rollFloorItemCount`, `SkillState.trained`, `untrainedSkills`,
`xpToNext`, `inventoryCap`, `dodgeCapPercent`, `lootStreamSalt` are each
