...
- `packages/app/lib/game/game_bloc.dart` — `pan` field, `MapPanned` event,
  `enemiesInSight`, `potionCount`, the refusal log line; delete
...
- `packages/app/test/grid_geometry_test.dart`, `packages/app/test/game_bloc_test.dart`,
  `packages/app/test/town_bloc_test.dart`, `packages/core/test/town/town_test.dart` —
  new behavior.
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: a camera origin at a fixed cell size"
```
...
Add to `game_bloc.dart`: `import 'dart:ui' show Offset;`, the `MapPanned` event class,
`pan` on `GameViewState` with the dartdoc below, `on<MapPanned>(_onMapPanned)` in the
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: a camera that follows the hero and pans by hand"
```
...
Refusal strings, verbatim from `step`: `'you are not carrying that'`,
`'${item.base.name} is not worn'`, `'both hands are on the weapon'`,
`'nothing is on your ${slot.name}'`, `'your hands are too full to stow it'`.
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "refactor: one home for the wear rule"
```
...
- Produces: `Transacted equipItem(Profile profile, String itemId)`,
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: wear and take off gear in town"
```
...
**Files:**
- Create: `packages/app/lib/game/item_presentation.dart`,
  `packages/app/test/item_presentation_test.dart`
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: read an item's numbers, grouped and stacked"
```
...
```bash
git add packages/app/lib/game packages/app/test/game_bloc_test.dart
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: say when something is watching, and count the potions"
```
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: a pack that explains what it is carrying"
```
...
**Interfaces:**
- Consumes: `equipItem`, `unequipItem` (Task 4); `packSections`, `statLine`,
  `wornDeltas`, `deltaLine` (Task 5); `TownRoom`, `Heading`, `NothingHere`, `Notice`
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "feat: a gear room in town"
```
...
git commit --author="fiatcode <fiatcode@posteo.com>" -m "docs: M2Q build report"
```
